...
http.listen(port, function () {
    ...
    mongoClient.connect("mongodb+srv://RitikTanwar:ritiktanwaradw136@cluster0.b507o.mongodb.net/my_social_network", function (error, client) {
        ...
        app.post("/signup", function (request, result) {
            var name = request.fields.namecd
        ...
        app.post("/acceptFriendRequest", function (request, result) {
            var accessToken = request.fields.accessToken;
            var _id = request.fields._id;
            // console.log("this is my accept id ", _id);
            database.collection("users").findOne({
                "accessToken": accessToken
            }, function (error, user) {
                if (user == null) {
                            ...
                            database.collection("users").updateOne({
                                ...
                                $push: {
                                    "notifications": {
                                        ...
                                        "content": me.name + "has accepted your friend request.",
                                        "profileImage": me.profileImage,
                                        "createdAt": new Date().getTime()
                                    }
                                }
        ...
        app.post("/unfriend", (request, result) => {
            var accessToken = request.fields.accessToken;
            var _id = request.fields._id;
            // console.log("this is my unfriend id ", _id);
            database.collection("users").findOne({
                "accessToken": accessToken
            }, function (error, user) {
                if (user == null) {
        ...
        app.post("/getFriendsChat", function (request, result) {
            ...
            database.collection("users").findOne({
                ...
                if (user == null) {
                    result.json = ({
                        "status": "error",
                        "message": "User has been logged. Please login again."
                    })
                }
                else {
                    var index = user.friends.findIndex(function (friends) {
                        return friends._id == _id
                    });
